As of MapLibre 6, using non-Web Mercator background tiles is very tricky, but this demo demonstrates that it is possible with enough hacks. We demonstrate a Plate Carree background map (sometimes referred to as unprojected EPSG:4326 /unprojected WGS84 / Equirectangular projection) in MapLibre, along with a style spec and 3D Tiles.
...
1. `alignGeoJSONWithPlateCarree` - For the style spec, we reproject the GeoJSON feature collection. Now the features are also sitting in the "wrong" place. We add the same offset the tiles have to the features, and now everything visually looks in place. (It seems that sometimes, two wrongs do make a right!)
