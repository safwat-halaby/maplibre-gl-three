...
If you've disabled vertical datums conversions, the orthometric height will be approximated to always equal the ellipsoidal height.
This may cause vertical offset issues if you're using 3d-tiles or other ECEF-based/ellipsoid-height based data sources along with a MapLibre 3d terrain, which uses orthometric height. */
...
/** Options for configurating maplibre-gl-three */
export interface ThreeDManagerOptions {
     ...
     * In the anchor-local system, the anchor is at 0,0,0 while the point to the right is at 5,0,0
...
/** An asset is something that is loaded to a ThreeLayer in addition to the raw ThreeJS primitives.
...
/** A 3dTiles asset that is loaded to a layer and rendered on the map. 
...
export interface ThreeDTilesAsset extends Asset {
    ...
    /** Sets an offset, in meters, relative to where the 3dtiles would be on earth if there were no offset.
     * Positive up offset moves the tiles away from earth's core.
    ...
    /** Returns the center of the containing sphere of the 3dtiles model. null if the 3d tiles aren't yet loaded.
...
/** A thin wrapper around a Maplibre custom layer and a ThreeJS scene.
 * It can be added to a Maplibre map just like any other layer.
 * The scene can be manipulated just like any threeJS scene. 
 * It exposes convenience functions for creating common assets such as 3dTiles, as well as lower-level threeJS primitives for rendering
 * threeJS can render, on a Maplibre custom layer. The camera is auto-synced with MapLibre's camera.
 * */
export interface ThreeLayer extends CustomLayerInterface {
    ...
    */
    isDestroyed(): boolean;
    /** Returns the threeJS scene. Objects within the scene are expected to have ECEF Vector3 coordinates.
     ...
     * Other than this, you may use the scene as you normally would use it in threeJS. 
    */
    getScene(): Scene;
    /** Returns the library-controlled threeJS camera in local space coordinates, updated during render.
     * Updating the camera is not advised. But it can be used for querying. E.g. raycasting.
     */
    getCamera(): PerspectiveCamera;
    /** Returns the threeJS WebGLRenderer. Returns null while the layer is detached from a map. */
    getRenderer(): WebGLRenderer | null;
    /** Adds 3dtiles to the map and returns the controlling asset. Resolves after asset setup, not after streaming finishes.
     * The asset is a thin wrapper around the "3d-tiles-renderer" library.
     */
